...
# easy map, not using it
range(QueenAnne$latitude)
range(QueenAnne$longitude)
mapplot<- ggplot(QueenAnne, aes(x=longitude, y=latitude))+geom_point(aes(col = room_type),alpha = .5)+
  scale_colour_manual(values = c('red', 'blue','green'))+xlim(c(-122.37,-122.35))+ylim(c(47.61,47.66))+
  ggtitle('The Map of Room Type')+theme(plot.title = element_text(hjust = 0.5),panel.grid.major = element_line(colour = NA))

mapplot

# beautiful map, use it
